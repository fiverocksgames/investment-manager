...
| REQ-PWA-001 | Provide a PWA baseline and generated web manifest | 1 | In Validation | Issue #3, `docs/ARCHITECTURE.md` | N/A | N/A | `vite-plugin-pwa`, `src/vite-env.d.ts` | Build passed; browser install and offline validation pending | #4 |
| REQ-DEPLOY-001 | Build and deploy the static application through GitHub Pages | 1 | Done | Issue #3, `docs/ARCHITECTURE.md` | N/A | N/A | Repository-relative Vite base path | Upstream Pages deployment verified | Upstream #4 |
| REQ-AUTH-001 | Authenticate users through Supabase Google login | 1 | Done | Issue #5, `docs/SUPABASE_SETUP.md`, `docs/SECURITY.md` | Auth identity managed by Supabase; user tables pending | Supabase Auth client | `src/auth/AuthProvider.tsx`, auth-aware `src/App.tsx` | Sign-in, callback, refresh persistence, browser-restart persistence, and sign-out verified | #6; upstream #4 |
| REQ-INFRA-002 | Provide guarded browser-safe Supabase configuration | 1 | Done | Issue #5, `.env.example`, `docs/SUPABASE_SETUP.md` | N/A — client configuration | `src/lib/supabase.ts` | Configuration state in app shell | Missing-variable fallback and configured production build verified | #6, #8; upstream #4 |
| REQ-SEC-001 | Separate public frontend identifiers from privileged credentials | 1 | Done | `docs/SECURITY.md`, `docs/SUPABASE_SETUP.md` | RLS required before user tables | No service-role client | Error and configuration states | Documentation, build, and production configuration verified | #6, #8; upstream #4 |
