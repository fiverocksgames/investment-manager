...
| REQ-DATA-002 | Preserve source, revision, quality, cutoff, freshness, and persisted immutable content | 2 | In Validation | Canonical metadata, FX provenance, deterministic snapshots, `SnapshotRepository`, PostgreSQL schema | Snapshot/persistence provenance tests; Python #78, Documentation #130; final-head gate via PR #47 | #16, #18, #43, #45, #47 |
...
| REQ-PROVIDER-002 | Explicit validation and failure classification | 2 | In Validation | Canonical failures, retry, transport diagnostics, typed FX/snapshot/persistence failures | Deterministic failure/conflict tests; Python #78, Documentation #130 | #18, #20, #29, #33, #35, #37, #39, #43, #45, #47 |
| REQ-OPS-002 | Idempotent ingestion and immutable snapshots | 2 | In Validation | `SourceSnapshotPublisher`, `SnapshotRepository`, `docs/SOURCE_SNAPSHOTS.md`, `docs/PERSISTENCE.md`, Supabase migration | Snapshot identity/cutoff tests plus atomic replay/conflict/rollback persistence tests; remote migration pending | #16, #45, #47 |
...
| REQ-MKT-002 | Expose source, retrieval time, freshness, FX direction, reproducible source-set identity, and persisted provenance | 2 | In Validation | Provider metadata, `FxPair`, snapshot checksum/UUID/cutoff, `numeric`/`timestamptz`/`jsonb` persistence | FX/snapshot provenance plus persistence serialization tests; Python #78, Documentation #130 | #20, #29, #31, #37, #39, #43, #45, #47 |
